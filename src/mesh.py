...
from utils import connected_components, read_off_file, vtk_point_locator
import vtk as _vtk
...
class Mesh:
    ...
    _edges = None
    _n_boundaries = None
    _vertex_adjacency_matrix = None
    _face_vertex_adjacency = None
    _edge_vertex_adjacency = None
    _edge_face_adjacency = None
    _face_centers = None
    _face_areas = None
    _vertex_areas = None
    _ivf = None
    ...
    def __init__(self, vertices, faces, color=None):
        ...
        self.points_locator = vtk_point_locator(self.poly_data)

    def face_points(self, indices):
        n, f = self.faces.T[0].T, self.faces.T[1:4]
        return np.ravel(f.T[indices])

    def set_vertices(self, pids, points):
        self.vertices[pids] = points
        self.poly_data.points[pids] = points

    def get_boundary(self, ids, n: int = 1):
        input_ids = ids
        for i in range(n):
            z = np.zeros(self.n_vertices)
            z[ids] = 1
            matrix = bsr_matrix(self.vertex_adjacency_matrix) * z
            ids = np.unique(np.where(matrix)[0])
        matrix[input_ids] = 0
        return np.unique(np.where(matrix)[0])
    ...
    def plot_function(self, f, name='f'):
        if len(f) == self.n_vertices:
            self.poly_data.point_data[name] = f
        elif len(f) == self.n_faces:
            self.poly_data.cell_data[name] = f
        self.poly_data.set_active_scalars(name)
        self.poly_data.plot(show_edges=True)
    ...
    def find_points_within_radius(self, point: np.ndarray, radius: float):
        ids = _vtk.vtkIdList()
        self.points_locator.FindPointsWithinRadius(radius, point, ids)

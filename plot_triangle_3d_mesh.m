function [p] = plot_triangle_3d_mesh(vertex, face, options)
...
if isempty(vertex_color) && isempty(face_color)
    p = patch('vertices',vertex,'faces',face, 'FaceColor', 'white');
end
...
if ~isempty(vertex_color) && isempty(face_color)
    ...
    p = patch('vertices',vertex,'faces',face, 'FaceVertexCData', vertex_color, 'FaceColor', 'interp');
    axis square;
    grid on;
    xlabel('X');
    ylabel('Y');
...
if isempty(vertex_color) && ~isempty(face_color)
    ...
    p = patch('vertices',vertex,'faces',face, 'FaceVertexCData', face_color, 'FaceColor', 'flat');
    axis square;
    grid on;
    xlabel('X');
    ylabel('Y');

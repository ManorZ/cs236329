...
import numpy as np
import glob
from utils import read_off_file, as_spherical, legendre
...
def calc_spherical_harmonic(theta, m, l):
    Ylm = np.sqrt((2 * l + 1) / (4 * np.pi) * (np.math.factorial(l - m) / np.math.factorial(l + m))) * legendre(l, np.cos(theta))
    return Ylm


def laplace_harmonics():
    sphere_file = r'./hw2_data/sphere_s2.off'
    vertices, faces, color = read_off_file(sphere_file)
    mesh = Mesh(faces=faces, vertices=vertices, color=color)
    p, t, r = as_spherical(mesh.vertices).T
    h0 = calc_spherical_harmonic(t, 0, 1)
    h = mesh.cot_weights()
    w, v = sp.linalg.eigs(h, 9, which='SM')
    m = v @ np.diag(np.real(w)) @ v.T
    vf = mesh.average_vertex_to_face(m[19])
    mesh.plot_function(np.real(vf), 'h0')
